    ...
    {   		
        ...
        */
        ...
        if(pid_fork == 0){
            ...
            if(background){
                printf("Comando %s ejecutado segundo plano con pid %d \n", inputBuffer, pid_fork); 
                waitpid(pid_fork, &status, NULL);
            }
            else{
                pid_wait = waitpid(pid_fork, &status, NULL);
                printf("Comando %s ejecutado en primer plano con pid %d. Estado finalizado. Info: %d. \n", inputBuffer, pid_fork, info);
            }
        } else {
